      ...
      <el-container style="radius: 30px;">
        ...
        <el-container>
          <el-main>
            ...
            <input type="button" @click="goback" value="返回" class="goback" style="color: black">
            <!-- 分割线 -->
            <el-divider></el-divider>
            ...
            <div class="text item">
              <el-form :model="ruleForm" :rules="rules" class="demo-ruleForm" label-width="68px" inline="true" ref="ruleForm" status-icon>
                ...
                <el-form-item class="lab" label="用户名" prop="username" >
                  <el-input auto-complete="off" id="name" ref="input2" v-model="ruleForm.username" @blur="checkname"></el-input>
                </el-form-item>
  ...
  export default {
    ...
    data() {
      ...
      var validateName = (rule, value, callback) => {   //rule验证规则，value输入的值，callback回调函数，如果出错则传入error对象
        const reg = /^[\w\u4e00-\u9fa5]{1,20}$/;/*用户名： 中文、A-Z a-z 0-9 _*/
        if (value === '') {
    ...
    methods: {
      checkname(){
        var data=qs.stringify({"user_name":this.ruleForm.username})
        this.axios.post('/user/existUserName',data,{headers: {'Content-Type': 'application/x-www-form-urlencoded'}}).then((res) => {
         alert(res.data[0])
      ...
      isname() {
        let reg = /^[\w\u4e00-\u9fa5]{1,20}$/;   /*用户名： 中文、A-Z a-z 0-9 _*/
